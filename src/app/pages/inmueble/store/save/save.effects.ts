...
import { InmuebleResponse, InmuebleCreateRequest } from './save.models';
...
export class SaveEffects {
  ...
  read: Observable<Action> = createEffect(() =>
    this.actions.pipe(
      ...
      switchMap(() =>
        this.httpClient
          .get<InmuebleResponse[]>(`${environment.url}/api/inmueble`)
          .pipe(
            delay(1000),
            map(
  ...
  create: Observable<Action> = createEffect(() =>
    this.actions.pipe(
      ...
      switchMap((request: InmuebleCreateRequest) =>
        this.httpClient
          .post<InmuebleResponse>(`${environment.url}/api/inmueble`, request)
          .pipe(
            delay(1000),
